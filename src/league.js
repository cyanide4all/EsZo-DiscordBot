...
module.exports = (client, riotRequest, firebaseDatabase) => {
  ...
  const handleDtoMatchData = (data, accountId) => {
    const participantId = data.participantIdentities.find(
      (e) => e.player.currentAccountId === accountId
    ...
    new Promise((resolve, reject) => {
      firebaseDatabase
        ...
        .then((snapshot) => {
          ...
          const prevBet = apuestas.find(
            (each) =>
              each.gameId === currentGame.gameId && each.author === authorId
          );
          resolve(prevBet);
        });
      ...
      client.on("message", async (message) => {
        ...
        if (regex.regexRiotRegister.test(message.content)) {
          ...
          const userName = userNameArr
            .splice(1, userNameArr.length - 1)
            .join(" ");
        ...
        if (regex.regexApuesta.test(message.content)) {
          try {
            ...
            try {
              ...
              const prevBet = await getPreviousBet(message.author.id, gameId);
              if (prevBet) {
                throw Error(
                  "Ya has apostado https://twitter.com/stopludopatia "
                );
              }
              ...
              const pollingFunc = () => {
                getMatchDatafromMatchId(gameId, target.riotAccountId)
                  .then(async (playerWon) => {
                    ...
                    if (type === playerWon) {
                      ...
                      setUserById(message.author.id, winner).then(() =>
              ...
              console.log(e)
              throw Error(
                "el jugador ese no esta en partida yokese, no me ra🅱es. Nunca supe como se escribía"
              );
            }
          } catch (e) {
            console.log(e)

...
module.exports = (client, firebaseDatabase) => {
  function cobrarPuntos(userId, isDisconect, cb) {
    firebaseDatabase
      ...
      .then((snapshot) => {
        ...
        if (userObj.lastConnect && userObj.lastConnect < now - 300000) {
          // 300000 = ms en 5 min
          newUser.points =
            userObj.points +
            Math.round(points5min * ((now - userObj.lastConnect) / 300000));
        }
    ...
    if (prevState.channel == null && newState.channel != null) {
      firebaseDatabase
        ...
        .then((snapshot) => {
          ...
          if (userObj.dailyConnect < now - 86400000) {
            // 86400000 = ms en 1 día
  ...
  client.on("messageReactionAdd", (reaction, author) => {
    if (
      reaction.emoji.id == 318132608057868309 &&
      reaction.message.author.id != author.id
    ) {
      firebaseDatabase
        .ref(`/users/${reaction.message.author.id}`)
        .once("value")
        .then((snapshot) => {
          const userObj = snapshot.val() ? snapshot.val() : defaultUserObj;
          const newUser = {
  ...
  client.on("message", (message) => {
    if (message.channel.id != 268398719802540032) {
        ...
        firebaseDatabase
          ...
          .then((snapshot) => {
            ...
            if (dataAsArray.length >= 3) {
              ...
              message.channel
                .send(`Muy poco capitalista hoy por aquí...`)
                .catch(console.log);
            }
          });
      }
        ...
        firebaseDatabase
          ...
          .then((snapshot) => {
            ...
            if (!data) {
              ...
              message
                .reply(`Tienes ${data.points} colacoins`)
                .catch(console.log);
            }
          });
      }
      ...
      if (message.content === "dame punto") {
        cobrarPuntos(
          message.author.id,
          !(message.member && message.member.voice.channelID),
          (newPoints) =>
            message
              .reply(`Ahora tienes ${newPoints} colacoins`)
              .catch(console.log),
        );
      }
      // Enviar punto
                          ...
                          firebaseDatabase
                            ...
                            .set(newData, (err) => {
                              if (err) {
                                ...
                                message
                                  .reply(
                                    `Has pagado ${cantidad} colacoins a <@!${destinatario}>`,
                                  )
                                  .catch(console.log);
                              }
                            });
